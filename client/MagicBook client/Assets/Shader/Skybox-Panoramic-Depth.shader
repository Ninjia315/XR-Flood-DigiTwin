...
Shader "Custom/PanoramicWithDepth" {
Properties {
    ...
    _DepthTex ("Depth  (HDR)", 2D) = "grey" {}
    _CamDepthTexture ("Cam Depth Texture", 2D) = "white" {}
}
...
SubShader {
    ...
    Pass {
        ...
        fixed4 frag (v2f i) : SV_Target
        {
            ...
            // Sample the environment depth texture
            half4 depthTex = tex2D(_DepthTex, tc);
            half3 environmentDepth = DecodeHDR(depthTex, _DepthTex_HDR);

            // Get the current fragment's depth value
            float2 screenUV = i.screenPos.xy / i.screenPos.w;
            //half4 sceneDepth = tex2D(_CameraDepthTexture, i.texcoord);
            float sceneDepth = Linear01Depth(tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(i.screenPos)));
            //float4 sceneDepth = (tex2D(_CamDepthTexture, screenUV));//SAMPLE_DEPTH_TEXTURE
            //float linearSceneDepth = LinearEyeDepth(sceneDepth);

            //return sceneDepth;//half4(linearSceneDepth, 0, 0, 1);

            float totalEnvironment = (environmentDepth.r + environmentDepth.g + environmentDepth.b) / 3;

            // Compare the environment depth with the scene depth
            if (sceneDepth < totalEnvironment)
            {
                // If occluded, return a specific color (e.g., red)
                //return half4(1,0,0, 1);
            }

            // Otherwise, return the skybox color
            //return sceneDepth;//half4(sceneDepth.a, 0, 0, 1);
            return half4(c, 1);
        }
        ENDCG
    }

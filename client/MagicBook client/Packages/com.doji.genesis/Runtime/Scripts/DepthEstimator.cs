...
namespace Genesis {
    public class DepthEstimator : IDisposable {
        ...
        public Texture2D PostProcessDepth(Texture2D tex)
        {
            var texPixels = tex.GetPixels32();
            var depthTexture = new Texture2D(tex.width, tex.height, TextureFormat.RFloat, mipChain: false, linear: true);
            depthTexture.wrapModeU = TextureWrapMode.Repeat;
            depthTexture.wrapModeV = TextureWrapMode.Clamp;
            depthTexture.SetPixels32(texPixels);
            depthTexture.Apply(false);
